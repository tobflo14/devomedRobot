...
int main(int argc, char** argv) {
              ...
              if (n < 0) {
               ...
               // std::cout << pid_constants[2] << std::endl;
                ...
                if (print_data.mutex.try_lock()) {
                  ...
                  //  for (int i = 0; i < print_data.data.size(); i) {
                  //    _dataTail = graph_update1(print_data.data[i], _dataTail, vertexDataBuffer);
                  //  }
  ...
  try {
    ...
    */
    ...
    auto cartesian_pose_callback = [=, &time, &inloop_data, &acceleration_limit, &jerk_limit, &last_force, &velocity_array, &print_data, &desired_force, &error, &past_error, &integral, &derivative, &vel_desired, &acc, &jerk, &pd, &pid_constants, &running, &timelapse]
     ...
     // std::cout << "inloop size" << inloop_data.size() << std::endl;
        if (print_data.mutex.try_lock()) {
          ...
          for (int i=0; i < inloop_data.size(); i++) {
            print_data.data.push_back(inloop_data[i]);
            print_data.data2 = acc[2];
          } 
          inloop_data.clear();
        print_data.mutex.unlock();
        }
